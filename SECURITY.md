...
- **MCP scoping:** configured MCP servers' tools are denied by default and allowed only for routines whose frontmatter grants the server (`mcp: [name]`); dry runs deny them regardless. This scopes which routines' context receives a server's third-party tool descriptions -- the standard MCP injection surface -- and which can invoke its tools. Connections still require the matching credential grant for the server's auth header.

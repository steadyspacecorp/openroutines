...
- **Filesystem confinement:** local model execution occurs in a disposable container. Production model execution uses per-attempt Unix permissions and, where available, Landlock to confine the run to its workspace and required runtime paths; the `landlock` compatibility profile requires Landlock rather than treating it as defense-in-depth. Writable paths are staged memory, the run temporary directory, a disposable per-attempt home, and `/dev`. Post-run bookkeeping re-executes the runtime as an ordinary supervisor child to read what the attempt consumed; it runs with an empty home and configuration directory, so it does not load plugins or configuration the run itself could write.
...
These controls are layered. The default `uid` isolation profile gives each production attempt a reserved Unix identity distinct from the supervisor and other attempts; Landlock adds filesystem confinement where available. The explicit `landlock` compatibility profile requires Landlock, confines attempts to one serial process group under the supervisor's identity, and denies process-group escape with seccomp. See [Operating in production](docs/operating.md#isolation-profiles) for the tradeoff.
...
- **The Landlock profile shares a Linux user.** On capability-restricted platforms using `isolation_profile: landlock`, opencode and the supervisor have the same user ID. Mandatory Landlock protects files and secrets, seccomp keeps descendants in the serial attempt's process group for cleanup, and the supervisor is non-dumpable, but same-UID model code may still disrupt supervisor-owned processes. The default `uid` profile does not have this limitation.

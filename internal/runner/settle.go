...
type Settlement struct {
	Outcome         Outcome
	Detail          string
	EventsDiscarded bool
	Commit          string
	Conflicts       []knowledge.Conflict
}

// Makes one attempt's end durable in knowledge -- the single settlement path
// for manual and scheduled runs. A rejected import downgrades the outcome to
// Crashed. beforeCommit, when set, runs before the settlement commit so caller
// bookkeeping rides the same commit. A Canceled attempt gets only its run
// record and no commit of its own -- the same logical run retries.
...
// Usage fields are per attempt (spend happens per attempt; retries would
// double-count a run-level figure) and absent means the runtime didn't report,
// never zero.

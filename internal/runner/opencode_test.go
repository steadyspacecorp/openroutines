...
func TestHostCaptureRunsWithAnEmptyHome(t *testing.T) {
	...
	out, err := hostOpencode{workspace: ws}.exec("session", "list")
	if err != nil {
		t.Fatal(err)
	}
...
func TestHostCaptureRefusesAHomeInsideTheWorkspace(t *testing.T) {
	...
	if _, err := (hostOpencode{workspace: ws}).exec("session", "list"); err == nil {
...
func TestHostCaptureSurvivesOpencodesLossyPipeWrites(t *testing.T) {
	...
	out, err := hostOpencode{workspace: t.TempDir()}.exec("export", "ses_x")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNativeCaptureSurvivesOpencodesLossyPipeWrites(t *testing.T) {
	fakeBin(t, "opencode", truncateOnPipe)
	out, err := nativeOpencode{workspace: t.TempDir()}.exec("export", "ses_x")
	if err != nil {
		t.Fatal(err)
	}
	if string(out) != `{"messages":[]}` {
...
func TestContainerCaptureClearsAPlantedLandingFile(t *testing.T) {
	...
	if _, err := (containerOpencode{workspace: ws, image: "img"}).exec("export", "ses_x"); err != nil {
		t.Fatal(err)
	}

...
// boot names it as the weaker choice, because a deployment that picked it
// once is never told again. Both supervisor keys are exposed the same way by
...
// A host where no rung works would otherwise leave an operator with no agent
// at all, so there is a way out -- and taking it is a deployment decision
// that has to be visible in the log every boot, because it is the one shape
// where a routine can reach another routine's credentials.
func TestBootRunsUnconfinedWhenTheOperatorDisablesTheSandbox(t *testing.T) {
	...
	// And the boot checks that exist to protect the sandbox's grant list stop
	// applying, rather than refusing a deployment over the smaller version of
	// a tradeoff its operator already made.
...
// File delivery keeps a key out of every environment, but it moves the
// question to where the file sits: a run shares the supervisor's uid, so a
// key under a path the sandbox grants is readable at any mode. That one is
// fatal rather than a warning -- unlike env delivery it is reachable by the
// very thing the sandbox exists to contain, and there is no degraded way to
// run an agent whose routines can read its master key.

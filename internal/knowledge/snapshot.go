...
// knowledge worktree.
func (m *Knowledge) FetchOriginSnapshot() (*OriginSnapshot, error) {
	if !m.HasOrigin() {
	...
	commit, err := git(m.repoDir, "rev-parse", "refs/remotes/origin/"+Branch)
	if err != nil {
	...
	if _, err := git(m.repoDir, "--work-tree="+dir, "checkout", "--quiet", commit, "--", "."); err != nil {
		_ = os.RemoveAll(dir)
		return nil, fmt.Errorf("exporting origin/%s: %w", Branch, err)
	}
	return &OriginSnapshot{Dir: dir, Commit: commit, FetchedAt: time.Now(), repoDir: m.repoDir}, nil
}
...
func (s *OriginSnapshot) Relation(m *Knowledge) SnapshotRelation {
	...
	local, err := git(m.Worktree(), "rev-parse", "HEAD")

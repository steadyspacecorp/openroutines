...
**Decision.** Runs are serial across routines: the supervisor executes one run at a time, in due order -- there is no thread pool. A routine that is still running when its next tick comes due is skipped, not queued: before spawning, the supervisor takes a non-blocking `flock` on a per-routine lock file, and a held lock means skip and log.
...
sequenceDiagram
    ...
    participant R as Routine run (opencode)
    participant M as memory/ (worktree of memory branch)
    participant O as Origin

    loop every minute
        S->>S: re-read frontmatter, compute due vs last_run
    end
    Note over S: one run at a time, in due order
    S->>S: flock routine
    S->>S: build clean env (declared credentials only)
    S->>R: spawn process group in Landlock sandbox,<br/>generated agent definition (declared skills only)
    R->>R: do the job (model, tools, skills)
    R->>M: append worklog / intentions / blockers,<br/>update own ledger
    R-->>S: exit (or timeout: SIGTERM group, grace, SIGKILL)
    alt run completed
        S->>M: commit routine writes + run record
    else run failed
        S->>M: discard writes, commit run record + blocker
    end
    S->>O: push memory branch (retry next run if it fails)

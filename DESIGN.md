...
**Decision.** Every routine run is filesystem-sandboxed by the supervisor using Landlock (Linux kernel 5.13+): before exec, the routine's process group is restricted to read access on the repo working tree -- minus undeclared skill directories and minus `.git` (history can contain deleted secrets; worktree metadata is the supervisor's, not the routine's) -- and write access to its staged memory copy (never the real worktree or any git metadata -- see "Memory syncs per run") and a per-run tmp dir. Nothing else, enforced at the syscall level, no root required. The generated opencode agent definitions additionally carry `read` permission patterns denying undeclared skill paths. **The sandbox fails closed**: if the host kernel lacks Landlock, `supervise` refuses to start -- running unsandboxed requires the deliberately ugly `OPENROUTINES_UNSAFE_NO_SANDBOX=1`, and `openroutines check` verifies effective Landlock support from inside the actual image when Docker is available (always true in CI and before a deploy), skipping that one probe with a loud warning when it isn't -- `check`'s other validations stay native and instant.

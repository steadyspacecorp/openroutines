...
| **Variables** -- non-secret configuration in `agent.yaml`, injected into every run as environment variables | The interface is identical to credentials, so the only question a value ever raises is "is this secret?" -- and the answer decides where it lives. |

...
**Decision.** Secrets ship encrypted in the repository, Rails-style: one encrypted file, one master key kept out of Git (a gitignored file locally, and a runtime mount or environment value in production).
Production resolves both master and deploy keys in the same order: a direct environment value, a path named by the file environment variable, then `master.key` / `deploy.key` in the agent root.
All three forms are accepted without logging a preference.

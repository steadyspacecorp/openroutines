...
Guidance for coding agents working on the Open Routines framework itself. (If you are working on a *scaffolded agent*, this is the wrong file -- see the AGENTS.md that `openroutines scaffold` placed in that repo.)

## What this is

Open Routines generates and runs single-purpose autonomous AI agents: one agent, one job description, one runtime, defined as a git repo and deployed as a Docker container. Routines are markdown files with frontmatter; execution goes through headless opencode; memory lives on a dedicated git branch.
...
- **At-least-once, never silently skipped.** Catch-up scheduling via persisted `last_run`; `flock` for overlap; process-group kills for timeouts; one atomic memory commit per completed run.
...
- **ORA** -- an Open Routines-generated agent

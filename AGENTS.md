...
- **The supervisor stays dumb.** Tick every minute, re-read frontmatter, run what's due -- into a bounded pool of run slots (`concurrency`, serial unless the agent opts in), with every knowledge-worktree operation serialized behind one lock. Enforcement lives elsewhere: skill, tool, web-access, and MCP scoping in generated opencode agent definitions, filesystem separation via per-attempt Unix identities with Landlock as defense-in-depth, credential scoping via built-from-scratch child environments. Do not add enforcement logic to the supervisor that a lower layer can provide.

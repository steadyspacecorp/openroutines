...
`routines run` starts opencode in a disposable Docker container. It is always a real run with the routine's credentials and tools. A manual run discards its staged knowledge writes and run record; pass `--write-knowledge` when the run should count.

...
Run `openroutines report` from an agent checkout to hear the check-in on demand: it syncs memory, asks before spending a run, then executes the check-in routine at log level warn so the report echoes to your terminal without lifecycle chatter. The run's memory writes are discarded, so nothing is consumed or committed -- the pending changes remain for the scheduled check-in. For the uncompressed view, the records are ordinary Markdown files under `memory/`.

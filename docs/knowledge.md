...
Run `openroutines knowledge` from an agent checkout to inspect a read-only snapshot fetched from origin: its interactive picker can summarize the current record with the default model, browse its files, or show branch statistics, without changing local `knowledge/`. The direct `list`, `show`, `stats`, and `summarize` subcommands provide the same surfaces to scripts and coding agents. Run `openroutines sync` when you want to adopt the latest knowledge into the local worktree, then curate the ordinary Markdown files there; `knowledge/ledgers/check-in.md` holds the latest check-in the agent delivered.

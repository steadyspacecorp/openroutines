...
**What you get by default asks for nothing.** Before the supervisor execs a model process it puts that process in a [Landlock](https://docs.kernel.org/userspace-api/landlock.html) domain granting the read-only OS, the run's own workspace, and nothing else -- no runtime flag, no capability, no privilege, so there is nothing to configure unless you want the stronger mechanism. That is what lets an agent deploy unmodified to a platform whose whole interface is a repository with a Dockerfile. Its one requirement is Landlock in the kernel: Linux 5.13 or newer, compiled in, which current distribution kernels have.

...
The decisions behind OpenRoutines, and why we made them.
The [README](../README.md) says what the framework does; this document says why it works the way it does.
...
**Decision.** A supervisor process in the container wakes every minute, re-reads routine frontmatter, and dispatches whatever is due.
Scheduling state is durable and per-routine (keyed by the routine's filename): a **watermark** -- the latest cron occurrence fully accounted for -- plus at most one **pending run**.

Each tick, per routine: if a pending run exists, retry it -- same `run_id`, new attempt.
Otherwise, find cron occurrences in `(watermark, now]`; if any exist, create a logical run with a fresh `run_id`, `scheduled_for` (earliest missed occurrence), and `covered_through` (latest -- missed firings collapse into one catch-up run, so an agent down for a week owes one run per routine, not seven).
**The pending record is committed and pushed before the routine executes: a logical run exists durably before it is allowed to act.**
The attempt is part of that record: immediately before it spawns anything, the executor *reserves* the attempt -- count incremented, time stamped -- and commits and pushes that reservation, so **no model process starts unless the attempt that spawned it is committed and pushed**.
The reservation belongs to the attempt, not to the tick that scheduled it: a run still queued behind another has spent nothing, so one routine that keeps killing its container cannot drain everything else's budget.
A reservation that never becomes a run (shutdown interrupts the attempt, the intent commit fails) is given back.
On success the watermark advances to `covered_through` and pending clears; on failure -- crash, timeout, shutdown -- pending survives and the same logical run retries on a later tick under the same `run_id` with a new attempt id.
After a bounded number of failed attempts (default 5), the supervisor abandons the run: human-owned task recorded, watermark advanced, pending cleared -- an unattended agent must not burn model spend retrying forever.
Abandonment happens at settlement when an attempt fails and reports back, at intent time when the budget is spent and nothing ever settled (the shape a routine leaves behind when it keeps taking its container down with it), and immediately when the failure is one **no retry can fix** -- five identical failures before the workspace even exists tell a person nothing the first one didn't.
The runner classifies that last case (an unreachable consumer cursor is the first of them), since it assembles the run and so knows why it could not; the bar is deliberately narrow -- the next attempt has to be *provably* the same, not merely likely to fail again -- or every stumble the environment causes becomes a run nobody will ever retry.
If origin is unreachable, no *new* logical run starts (an identity that isn't durable is how duplicates happen): the supervisor records the condition as a human-owned task -- written locally while the outage lasts and published when origin returns, see "Alerting degrades to durability when the datastore breaks" -- and catch-up absorbs the pause.

Abandonment has a second tier: a **circuit breaker** per routine.
Per-run abandonment caps one logical run at five attempts, but the schedule keeps minting new runs, so a routine failing *persistently* (dead model, revoked credential) would otherwise grind attempts -- and spend -- indefinitely.
After three consecutive abandonments the routine enters a cool-down: no new logical runs until it elapses (one hour, doubling per further abandonment, capped at 24h), with an event recorded at each trip, and any successful run resets the breaker.
Firings missed during cool-down collapse into one catch-up run when it ends.
(Found by the first production soak: an eight-hour model outage produced sixteen abandoned runs of continuous retry grind.)

`openroutines status` reports only what a tick will actually do: the schedulability predicate is shared with the scan (`supervisor.Schedulable`) rather than restated, a routine in cool-down is given no next-fire time, and a pending run is labelled with what becomes of it -- backing off, in flight, due, budget spent, or held -- instead of an unconditional next-attempt time.
A reserved attempt and a failed one leave the state file identical, so a run is only reported in flight when `runs.jsonl` lacks the settlement record that would have ended it, never on the strength of a missing log.
...
Cron is evaluated in the agent's timezone (`timezone:` in `openroutines.yml`) -- never the container's local zone, never whatever zone a persisted timestamp carries.
A fire time is a wall-clock promise: `0 6 * * *` for an `America/New_York` agent means 06:00 in New York on both sides of a DST transition.
The schedule package owns the binding -- a cron expression is parsed *into* a location, and every scan normalizes its inputs into that location -- so no caller can supply an unbound spec.
That matters because the scheduler scans from persisted timestamps: a watermark round-trips through the state file as a fixed UTC offset (Go fabricates a zone on unmarshal), and an unbound spec is evaluated in its argument's zone, so the schedule would freeze at the offset in force when state was last written and drift an hour at the next transition, permanently.
The same binding computes the forward schedule injected into runs and the next-fire line in `status`, so what a routine is told and what the supervisor dispatches cannot disagree.

**Why.** Re-reading frontmatter every tick means there is no registration or compilation step anywhere -- the files are the schedule, the supervisor carries zero derived state to invalidate, and local runs pick up edits instantly.
Catch-up semantics mean a routine whose moment passes during a redeploy or downtime fires late instead of never; for an unattended agent, "late" is recoverable and "silently skipped" is not.
The consequence is **at-least-once** execution -- which applies to *external side effects* too: discarding a failed attempt's knowledge writes does not unsend an email or un-open a PR.

Persist-before-act is what makes the run ID an idempotency key worth having.
Without it, a container can act, crash before recording anything, restart with no knowledge of the old run, and repeat the action under a fresh identity -- the exact duplicate the key was meant to prevent.
With the pending record pushed first, every retry carries the same `run_id`; routines that act on the world should use it (as an idempotency key where the API accepts one, or stamped into created objects and searched for before acting) and record completed external actions in their ledger.
It makes deduplication *possible*, not automatic -- an API with no idempotency support can still see a duplicate in the window between acting and recording.

The reporting rule is there because the failure mode is specific and shipped twice: a routine four attempts into a retry, sitting out a 24h cool-down, or still holding a pending run after being deactivated, rendered exactly like a healthy one, with a confident next time it would not honor.
An operator who cannot see that a routine is held has no reason to look.
Sharing the schedulability predicate with the scan is what stops the two drifting apart a third time: a surface that re-derives "will this run" will eventually be wrong, in the reassuring direction.

Attempt accounting has to be durable for the same reason the identity does.
Counting an attempt only once it comes back means a routine that reliably kills its container (OOM, host loss, platform eviction -- production recovery is container *replacement*) is never observed to have attempted anything; the replacement materializes knowledge from origin, reads `attempts: 0`, and dispatches again, indefinitely, leaving `MaxAttempts`, abandonment, and the circuit breaker unreachable for the one failure mode they most need to bound.
Reserving before dispatch inverts the error: a lost container costs one attempt instead of none, and giving back reservations that never became runs keeps a rolling deploy from spending a run's retries.
Reserving *per attempt* rather than per tick costs one small commit per dispatch and buys the rest: a tick can have several routines due at once and the pool is bounded, so reserving the whole batch up front would charge a lost container's attempt to every routine still waiting for a slot, abandoning routines on the evidence of attempts they never made.

Persist-before-act is likewise a property of the data, not of control flow: the intent writes are ordinary files in the knowledge worktree, so a commit takes whatever the worktree carries rather than only what this pass changed.
The failure that motivates it: a tick writes state, its commit fails, and the next tick sees a pending run, mints nothing, and dispatches under an identity that exists only on this container's disk -- indistinguishable from a clean tick in the control flow, obvious in the dirty worktree.
A commit that *fails* halts dispatch and is recorded as a human-owned task, the same as an unreachable origin: a supervisor that cannot record what it is about to do must not do it.

## Triggers: outbound wake-ups, not ingress and not deliveries

**Decision.** A routine can declare a `trigger` alongside or instead of its `schedule`: a cheap, outbound change-detection poll that makes the routine due.
`poll` names the URL; an optional `credential` must also appear in the routine's `credentials` and authenticates the request as a bearer.
A raw credential is sent verbatim.
A typed credential is derived fresh for each poll by the trusted supervisor, only the type's explicit bearer material is sent, and cleanup runs immediately after the request (including revoking a GitHub App installation token); the stored root secret never becomes an HTTP header.
An optional `select` extracts one value from a JSON response by RFC 6901 pointer; `interval` sets the poll cadence (default 5m, floor one tick).
A trigger carries no payload: when it fires, the routine runs exactly as it would from a schedule firing and pulls its actual work through its own skills.
**Triggers are best-effort latency reduction; the schedule remains the correctness backstop** (`check` warns on a trigger-only routine with no heartbeat schedule).

Change detection needs two declarative mechanisms and no more.
Conditional GET -- the supervisor stores and replays `ETag`/`Last-Modified`; a 304 means no news -- covers services that support the web's own change-detection protocol (GitHub's notifications API is the canonical case).
`select` covers the rest: nearly every API has a "latest item" query whose full response isn't byte-stable but contains one monotonic value (Slack's `conversations.history?limit=1` → `/messages/0/ts`).
Without `select`, the raw body is reduced to a digest and compared.
A JSON pointer is deliberately the ceiling of the power level: a static path, not a query language -- no predicates, no code execution, and the framework never learns any service's schema.
Webhook-only services with no pollable read surface are out of scope by design -- the answer there is a user-owned bridge (anything that can flip a byte at a pollable URL), never webhook ingestion in the framework.

Mechanically, triggers reuse the run machinery whole.
The tick evaluates a routine's trigger only when no cron firing is due and the interval has elapsed; on change it mints a logical run through the existing pipeline -- pending record, persist-before-act intent commit (which also carries the newly observed value), retries under the same run id, abandonment, the circuit breaker, coalescing.
Durable trigger state (`state/triggers/<routine>.json`: last value plus conditional-request validators) lives with the rest of supervisor-owned scheduling state; the last-poll clock is supervisor knowledge only, since persisting it would dirty the knowledge worktree on every poll.
The first observation establishes a baseline and never fires -- a new trigger starts at the present, mirroring how a new consumer starts at the current commit.
...
One `interval` governs polls, fires, and reply latency alike -- a poll is the only fire opportunity, so a single knob bounds request rate (courtesy to the polled service), model spend (a flapping or malicious endpoint fires at most once per interval), and responsiveness.
An earlier draft split the last concern into a separate `cooldown`; it was redundant and produced inconsistent reply latency -- an agent that answers within a minute once and within five the next reads as broken, where "replies within N" is a contract someone can feel.

**Why.** Scheduled-only routines cap responsiveness: an inbox-style routine must either poll on a tight schedule (a model run per interval to mostly find nothing) or accept the interval as reply latency -- our own dogfooding paid ~96 runs a day for 15-minute replies.
The trap in fixing that is letting the trigger become a message channel: payloads, signatures, ordering, replay.
Defining it as a bare wake-up dissolves all of it -- a spurious fire is harmless (the routine finds nothing and exits), a missed fire is covered by the heartbeat, and no external payload can ever cross into model context via the trigger path, because there is no payload.

The genuinely new surface is that the supervisor makes an outbound call with a routine's credential -- the one place it uses one itself.
It is bounded the way the rest of the design bounds things: the URL is a frontmatter grant (versioned, reviewed, no dynamic destinations), the request is a bare stdlib GET (no shell, no redirects, short timeout, capped reads), and the response is never logged raw, never interpreted beyond the bounded parse-and-extract, never shown to the model.
Typed derivation does not widen it: the same built-in provider code mints a bearer for one poll and disposes of it before returning to the tick; generic supervisor code never selects an arbitrary injected environment value and never sends the stored root.

A **webhook receiver** was rejected on cost accounting, not securability.
Webhooks are lossy by contract (senders retry on a budget and give up; Slack disables endpoints that fail repeatedly), and an ORA is deliberately allowed to be down -- so polling reconciliation is required anyway and the receiver is only a latency layer on top.
That layer's sole purchase, sub-minute latency, doesn't cover its costs: an HTTP listener, TLS stack, and per-service signature schemes inside the process that holds the master and deploy keys; a standing untrusted payload one step from model context; and deployment prerequisites (public ingress, DNS, certificates) that break "runs anywhere a container runs" -- half our targets are NAT'd.
Zero ingress is an asset in itself: "the container listens on nothing" is a one-sentence trust story.
When latency pressure genuinely arrives, the escape hatch is an **outbound stream** (`trigger: {stream: ...}`, SSE), not a port -- deferred because a tuned-down interval already covers agent-reply latency, SSE-native services are rare, and the tempting cases (Slack Socket Mode) would put proprietary protocol knowledge in the trusted binary.
Nothing shipped forecloses it: `trigger` taking `poll` today and `stream` someday is why it's a sub-key, and a service's poll cursor doubles as the stream's resume token.
...
Before a reporting run, the runtime fixes a boundary at the knowledge branch's current commit, walks every commit between the consumer's **cursor** and that boundary, and injects the additions and transitions as a read-only `changes.md` in the run workspace.
When the routine's work has covered the whole change set, it creates a `CONSUMED` marker file inside its staged knowledge directory -- the one workspace path the filesystem sandbox leaves writable.
...
**Decision.** Terms of art are artifact names; everything else is plain English.
The collective name is the README's -- **teamwork primitives** (never "teamwork loop" or "teamwork system") -- and the vocabulary is exactly the artifacts a user can open -- `events.md`, `tasks.md`, `context.md`, ledgers, `changes.md`, `schedule.md`, the check-in -- plus two frontmatter keys, `teamwork` (how a routine participates) and `reports` (the routine reports).
Knowledge is the substrate and the teamwork primitives are built on part of it: events (what happened), the schedule (what's ahead), and the report (telling the team).
Nothing is coined: "forecast" survives only as an ordinary English verb (the forward-looking artifact is the schedule, matching `schedule.md` and the `schedule:` key), and "inbox" is retired (the injected file is the unconsumed slice of the change feed, and routines watch real inboxes).
Retired keys (`events`, `consumes`) hard-error in `check` with the correct mapping, never a silent reinterpretation; the mapping for `events:` is counterintuitive (`events: false` -> `teamwork: off`, not `teamwork: events`), which is exactly why it is spelled out.

**Why.** The teamwork names accreted piecemeal as features landed: `events: false` didn't mean what it said, `consumes: knowledge` named a mechanism rather than a role and dressed a boolean up as an enum, and `inbox.md` was the system's last metaphor.
Each name made sense when it landed; together they asked the user to learn a vocabulary the system never decided on.
The docs prose had already converged on the right words -- "reporting routine", "the schedule", "changes" -- so the fix was to promote the words the prose and code already used into the identifiers, and to coin nothing.
Keeping the term set closed under "artifact you can open, or plain English" is what keeps it from accreting again.
...
**Decision.** Run output never enters the log stream, and the framework writes no run artifacts of its own.
The run's record is two opencode surfaces, each reached the way opencode itself offers it.

First, opencode's diagnostic log: every run passes `--print-logs --log-level=<the process's own effective level>`, and the runner passes each line opencode writes to stderr into the log stream verbatim, with the attempt's identity appended (`routine=`, `run_id=`).
Appending is the whole treatment -- logfmt is a bag of `key=value` pairs, so decoration is two more pairs, never a parse -- and the line stays opencode's record: its own timestamp (UTC, where the supervisor's render in the agent's zone), its own level, its own field names.
The stream arrives pre-gated because the run was asked for the level this process keeps, and pre-scrubbed because the passthrough sits behind its own scrub writer.
It does not go through slog: a deployment that swaps the handler to JSON keeps opencode's lines logfmt, which is the honest rendering of another program's log.
opencode renders run *progress* on stderr as well -- banner, tool traces, echoed tool output -- so an unattended run also passes `--format json`, moving that rendering to stdout where run output is already discarded; a manual run keeps the default rendering, because the person watching wants the progress.

Second, session history: when `OPENROUTINES_SESSION_DIR` designates operator storage -- typically a mounted volume, so history survives redeploys -- the supervisor asks opencode for the attempt's sessions (`session list`) and saves `opencode export` of each to `<dir>/<run_id>.<attempt_id>/<session_id>.json` when the attempt ends, whatever the outcome.
Unset means nothing is written; an env var rather than configuration for the same reason as the log-level override -- storage is wired up where the container is defined, not in the repo.
The model process never touches the volume: the exports run in the supervisor's process after the run, through the same capture-home hygiene as usage capture, so one run can never reach another's history.
The sessions are fetched once per attempt and shared, so usage capture and the export can never disagree about one attempt.
Every capture exec connects opencode's stdout to a file, never a pipe -- opencode's CLI exits without draining its final stdout write, and stream writes to a pipe are asynchronous, so a large document read through a pipe arrives cut at a 64 KiB boundary with exit code 0 (observed in production truncating exported sessions mid-string); writes to a file are synchronous.
The local-container exec cannot interpose a file from outside -- docker's stdout is a pipe end to end -- so there the redirect happens inside the container, with the landing file minted and held open by the supervisor and read back through that same descriptor: the accepted plugin residual runs code inside the capture container, and re-resolving the model-writable path after the exec would let that code swap in a symlink and read the host with the supervisor's eyes.
The fetch refuses any response that is not complete JSON, retrying once, as the backstop for every surface, and the ids the list returns come out of a model-writable store, so an id that could not serve as a plain filename exports nothing.
Exports are deliberately verbatim -- redaction is an output rule, and a framework that edits stored history is rewriting the record it exists to preserve.
A retried attempt reuses its `run_id.attempt_id`, so its export replaces that directory instead of merging into it, and an operator who designates a session dir owns what lands there, sizing and retention included; nothing is pruned.
The supervisor's outcome records name the attempt's directory (`sessions=...`); an export that fails partway still names its directory, while one that lands no file at all names nothing, because an empty directory is not a record.
The passthrough doubles as the failure evidence: a failed attempt's diagnostics are already in the log under the run's identity, session storage or not, so an agent never fails invisibly.
Run output reaches a person in exactly one place -- a manual `openroutines routines run` echoes it, scrubbed, to the terminal.

**Why.** This replaced a predecessor that copied the `opencode/` tree out of the attempt's data home verbatim, on the premise that opencode persisted readable artifacts there.
opencode moved session persistence into its database, which broke the premise twice over: the copied tree preserved an opaque database file rather than replayable history, and nothing in it was worth a filesystem-walking copier with symlink discipline when opencode will simply hand over the record.
Both surfaces now come from opencode's own interfaces, which survive its storage format changing again.
For the log, passthrough-with-appended-identity rather than adoption into slog records: slog decorates only at emission (`Logger.With`) and the stdlib has no logfmt ingester, so making opencode's lines into our records means hand-parsing another program's log format.
Appending two pairs buys the property that matters -- filter by `routine=`/`run_id=` while concurrent attempts interleave -- and what it gives up (our timestamp zone, `msg=` vs `message=`, handler-level gating) was never load-bearing, because the level is enforced at the source by the flag the run is spawned with.
For history, `opencode export` is the session in its documented, replayable form, and one file per session keeps a run's subagent sessions distinct.
The trade is that history lands when the attempt ends rather than growing live in storage; a run still in flight is watched through the log stream it is now part of.

## The log is structured: slog records, logfmt output

**Decision.** The framework logs through `log/slog` into one process-wide logger, rendered by the stdlib's own `slog.TextHandler`.
The handler is installed once, at the logging package's load, and reads its knobs -- destination, level, timezone -- live, so nothing ever logs through slog's stock unscrubbed handler and there is no setup call to forget; the CLI dispatch assigns level and timezone from the agent's configuration before any command runs, so a new subcommand gets configured logging without knowing the logging package exists.
The destination is stderr everywhere -- the stream Unix reserves for diagnostics and the one Go's own log package defaults to -- leaving stdout to carry run output alone, and opencode's passed-through lines land there too.
Everything afterwards calls `slog.Info`/`Warn`/`Error` directly; no logger is stored on a struct or threaded through a call path.
The output is logfmt, so every part of a record is a field and none of it is interpolated into prose: `time=2026-07-31T14:52:31.902-04:00 level=ERROR msg="attempt failed -- will retry" routine=check-in run_id=run_abc detail="exit status 1"`.
Messages are short constants; variable data is attributes.
A routine's identity is bound with `slog.With` (`r.Log()`) rather than repeated by each call site, and travels as an ordinary `routine=` field.
Timestamps render in the agent's timezone via the handler's `ReplaceAttr` hook -- the only customization the framework applies.

The level is the handler's gate and nothing else's.
No call site decides whether it is allowed to speak, and the level is a `slog.Level` throughout rather than a framework enum, since slog already orders these four names.
Its resolution is the `OPENROUTINES_LOG_LEVEL` override -- the only level knob; there is no configuration-file setting, so flipping a live container's level never waits on a redeploy -- then a default split by where the process runs: info inside the production container, whose log stream is an unattended agent's only interface, and warn everywhere else, where the person at a local terminal is watching the run's own output.
opencode's passed-through lines are outside the handler but not outside the discipline -- they arrive pre-gated at the source (see "Run history") -- and run *output* never asks the level anything, because it is not log records at all.

Redaction is nobody's decision either: a secret registers with one process-wide scrub registry at the door it enters through -- the master-key load, the store decrypt, the `Derive` mint, the deploy-key configure -- and every consumer (the log writer, the opencode-log passthrough, the manual echo, the knowledge-append seam) redacts from that registry.
Registration lives in those four chokepoints, never in a credential provider or a call site: a value can only leak from a process that materialized it, materializing is what registers it, and a new credential type gets scrubbing without writing any.
Process-lifetime secrets register by name and stay; a derived bearer registers per mint and its cleanup releases the registration, because concurrent runs minting the same credential hold distinct bearers -- a name-keyed overwrite would stop redacting one run's still-active material -- and releasing at cleanup keeps the registry bounded by live material rather than history.
Session-derived failure text is redacted the moment it is lifted from the session record, while the mint's registration is still live: that text outlives the registration -- in events, the run record, the manual echo -- and a consumer cannot redact from an entry already released.

**Why.** The level existed as a filter long before it existed as a field: three helpers gated on it and then dropped it, so at the default level a failed run and a registered routine rendered identically, with five hand-written `WARNING:` prefixes doing the job where somebody remembered.
That is a real defect in a system whose only interface is its logs, and the fix is the ordinary Go one: log at a level, let the handler decide.
Using the stdlib handler rather than writing one is the same instinct applied to the format: a hand-rolled renderer that puts the level in a padded column and the routine name in front of the message reads marginally better and is not actually structured -- the routine stops being a field the moment it is interpolated into the message.
Logfmt is a real standard, it is what `slog.TextHandler` already emits, and it is still perfectly readable; the alternative was ~250 lines of handler plus its conformance tests earning a column layout, and `slog.JSONHandler` remains a one-line substitution.
A single global logger is the honest shape for this program: one supervisor per container and one run per manual invocation, so a logger field was plumbing that carried no information -- and routing every line through one installed destination is what makes the scrubbing guarantee total, since no path can log an injected secret by holding a writer that skips the wrapper.
Structure earns its place at the call sites too: the run path repeated the routine name and run id into roughly forty format strings, which `With` binds once.
Emitting a timestamp despite the platform stamping its own is deliberate -- they measure different things (when the event happened, versus when it was ingested), and the app's is the one that survives being copied into an issue.
It renders in the agent's zone because the schedule is a wall-clock promise in that zone, and correlating a log line with a cron expression should not require arithmetic.

## CLI color: stdlib ANSI, never load-bearing

**Decision.** Command output styles itself with hand-rolled ANSI escapes -- bold section headers, green ✓, red ✗, yellow !, dim ○ -- decided once at load: color only when stdout is a terminal, `NO_COLOR` is unset, and `TERM` is not `dumb`.
Color is never load-bearing: every mark and word carries its information unstyled, so a pipe, a CI log, or a screen reader loses emphasis and nothing else.
There is no styling dependency and no TUI framework, and this is the ceiling: anything richer belongs in tooling outside this binary.

**Why.** The obvious move is a terminal-styling library, and the reason not to is the same one that keeps the dependency count near one: the binary that prints a pretty ✓ is the supervisor, the trusted component whose dependency tree is its attack surface -- a styling stack is a dozen modules of supply-chain exposure spent on cosmetics.
The output surface is a handful of marks and headers; eighty lines of stdlib covers it.
The interactive-dashboard itch is real but belongs to operator tooling that never ships in an agent image, where a TUI framework costs what it should.
...
Environment delivery remains available for platforms that cannot mount secrets, with a weaker process-exposure posture -- and the supervisor warns at boot when that is what it got, since a deployment that chose it once is otherwise never told again.
A routine's process environment receives only the credentials its frontmatter declares (`slack_webhook` → `SLACK_WEBHOOK`), decrypted in knowledge at spawn time -- never written to disk.
...
**Decision.** The supervisor -- tick loop, locks, timeouts, spawn, git sync -- is written in Go, with a dependency tree you can list from knowledge: a cron-expression parser, a YAML parser, terminal input for credential prompts, and Landlock bindings.

**Why.** The supervisor is the trusted component: it holds the master key and the deploy key, so its dependency tree is its attack surface.
Go's stdlib covers processes, signals, and locking natively, compiles to a static binary, and keeps the image minimal (supervisor + opencode + git, distroless-friendly).
It is also the lingua franca of exactly this genre of infrastructure -- the audience that runs containers expects the runner to be a small static binary they can read in one sitting.
TypeScript would have meant an npm tree inside the trust boundary; Rust buys safety this program doesn't need at a contribution cost it would feel.

## The CLI rejects flags it doesn't recognize, and `--help` always works

**Decision.** Every subcommand parses its own arguments through a shared helper (`internal/cli/flags.go`) that splits positional arguments from flags, fails on anything flag-shaped it doesn't know about, and recognizes `--help`/`-h` anywhere in the arguments before validating anything else.
`configure` additionally refuses to run against a non-interactive stdin that runs out mid-prompt (EOF before every field is answered) unless `--yes` is passed -- the same instinct `plugin add` and `plugin update` already had for a bundle install, extended to the command that generates the master key and writes the credentials store.

**Why.** An unrecognized flag used to be silently ignored -- `openroutines status --bogus` printed status as if `--bogus` had never been typed -- and `--help` wasn't handled by any subcommand, so `openroutines check --help` ran the check instead of describing it (issue #67).
Silently ignoring a flag someone typed is worse than an error: they believe it took effect.
The sharper case is `configure`: typed with piped or EOF stdin, every prompt silently accepted its default, generating a master key and writing `.openroutines/credentials.yml.enc` with no confirmation -- harmless against an empty store, a real risk against an agent that already has credentials.
A fully piped set of real answers (the pattern `bin/smoke` uses) still works without `--yes`: the guard triggers on stdin actually running dry, not merely on stdin being a pipe.

## Framework code is versioned out of the agent repo

**Decision.** All framework logic lives in one released Go binary -- `openroutines` -- whose subcommands include the supervisor (`openroutines supervise`, the container entrypoint) as well as `scaffold`, `configure`, `status`, `routines`, `skills`, `credentials`, and `update`.
An agent repo carries only a version pin (`.openroutines/version`) and a Dockerfile that installs the pinned release.
Locally you install `openroutines` once (installer script or Homebrew); the binary reads the repo's pin and warns on mismatch on every command, and `check` names the mismatch as its first problem -- a mismatched binary reads the repo through the wrong schema, so every later finding is suspect until the versions agree.
Source builds are exempt from both, because development runs against pinned agents on purpose.
(Bundler-style re-exec of the pinned version is a possible later upgrade.)
There are no `bin/` shims.
`openroutines update` bumps the pin, applies template changes to framework-owned files interactively (`rails app:update`-style), and leaves a single reviewable commit.
Framework-owned means files users shouldn't hand-edit (the Dockerfile, ignore files, `AGENTS.md`); `opencode.json` is not among them and update never rewrites it, after an early update offered to clobber a user's provider config.
The config boundary follows interpretation: `opencode.json` holds what only the harness reads -- permissions and custom provider endpoints -- while model *choice* stays in `openroutines.yml` and frontmatter, which the framework resolves per routine and keys credential injection off.
`check` warns on model config in `opencode.json` and on a provider no model references.

Deployment distributes the framework as **released binaries behind a public installer**: a pushed tag runs goreleaser in CI, which attaches cross-compiled binaries and checksums to the GitHub release, and the pages workflow republishes them -- with `install.sh` and a `version.txt` -- to GitHub Pages, which stays public even while the repository is private.
The agent's Dockerfile is entirely its own: a root-installed `base` (debian + git/gh/jq/ssh + pinned opencode, all in system paths the agent user cannot write), a `runtime` stage that drops to the agent user for local runs, and an `agent` stage that installs the pinned release via the installer, copies the repo to `/agent`, and sets `openroutines supervise` as the entrypoint.
`openroutines update` keeps the Dockerfile's `ARG OPENROUTINES_VERSION` in sync with the version pin.
This replaces the earlier GHCR base image, which made every deploy path authenticate against a private registry -- flyctl's builders cannot, forcing a build-push-deploy workaround -- while the installer's checksum verification does the same integrity job and a self-contained Dockerfile lets standard platform deploys work unmodified.

**Why.** Template repos rot: agent repos are born from `template/` and immediately diverge, so shipping framework *code* into them strands every existing agent on the version it was scaffolded with -- and fork-and-merge from upstream conflicts the moment a user touches anything.
Shipping a version *pin* instead makes drift structurally impossible for the whole binary surface; only the Dockerfile (a file users rarely edit) ever needs a guided merge.
The container is where the pin is enforced -- the Dockerfile installs that exact release, so deployed behavior is reproducible; locally the binary checks the pin and complains about skew.
Updating is one commit; rolling back an update is `git revert`.
The costs: a one-time local install step (no clone-and-run), and maintaining tagged releases with prebuilt binaries, which a small release script makes cheap.

## CLAUDE.md is a symlink to AGENTS.md

**Decision.** `scaffold` creates `CLAUDE.md` as a relative symlink to `AGENTS.md`.
`AGENTS.md` stays the single source of truth and the only rules file on the framework-owned list; `update` never touches the link, because rewrites of the target flow through it.
The link is minted by scaffold code rather than shipped in `template/` because `go:embed` refuses symlinks.
No Windows accommodation is carried: released binaries target darwin and linux only, and an agent repo checked out where symlinks degrade to plain files loses convenience, not correctness.

**Why.** "The coding agent of your choice" has to include Claude Code, which reads `CLAUDE.md` and not `AGENTS.md` -- its documentation recommends exactly this symlink.
A copy would drift: the first `update` that rewrote `AGENTS.md` would leave `CLAUDE.md` giving stale, actively wrong guidance, and preventing that would mean adding the copy to the framework-owned list -- bookkeeping the symlink makes unnecessary.
Nothing about the runtime changes: the run workspace is built by allow-list, so `CLAUDE.md` is excluded from runs by construction, exactly as `AGENTS.md` already is.
...
**Decision.** Routines never touch the real knowledge worktree.
Before each attempt, the supervisor snapshots the knowledge files into a plain, disposable **staging directory** -- no git metadata anywhere in it -- and that staging copy is what the routine sees and writes as `knowledge/`.
After a successful attempt, the supervisor validates the staged tree and imports the diff into the supervisor-only worktree, adds the run record, commits, and pushes.
A failed attempt's staging is discarded whole -- cleanup is deleting a directory, not surgery on a worktree.

Validation rejects the entire run if staged knowledge contains anything but regular files under the expected layout: no symlinks or hard links, no git control files (`.git`, `.gitattributes`, `.gitmodules`, hooks), no edits to supervisor-owned state (run records, scheduling state), and enforced limits on file sizes, counts, and path depth.
Validation is a walk over a directory the supervisor does not own, so it is not the last word: the import re-applies the same policy to what it actually opens -- inside a root that confines every path component to the staging tree, deciding on the descriptor it reads from that the source is an ordinary file with no second name, and bounding the copy by the same size cap.
A path that was a regular file when validation walked it and is a symlink, a hard link, or a supervisor-owned path when the import opens it fails the run at the open, not silently at the read.
Because that rejection lands mid-tree, the copy is assembled beside the worktree and promoted into it only once every staged file has passed, so a rejected import leaves the worktree exactly as it found it.
The supervisor invokes git with pinned, hermetic configuration (`GIT_CONFIG_NOSYSTEM=1`, global config disabled, hooks path empty, the `file` transport confined to same-user paths), an environment constructed from a small allowlist rather than inherited, and argument-safe invocation -- never through a shell.

Import has one enforced precondition: the human-curated knowledge files must be clean in the worktree.
Uncommitted curation has no reflog to recover from, so a run that would import over it fails instead -- commit or discard the edits and re-run.
(Supervisor-owned paths are exempt: they carry the attempt's own in-flight bookkeeping.)
The precondition protects local `routines run`, which is where a person actually edits knowledge.
Under a supervisor it rarely triggers, because the tick commits whatever the worktree carries before dispatching: curation left uncommitted inside a running container is committed and pushed rather than refused -- persist-before-act cannot be conditional on the tree being clean, and a commit is recoverable in a way an uncommitted edit is not.

Each attempt therefore produces two commits: a small **intent commit** immediately before it starts (the attempt's reservation) and a **completion commit** after (imported writes plus run record on success; run record plus failure event on failure).
A tick's own scheduling bookkeeping -- newly minted pending records, refreshed trigger baselines, abandonments -- commits once per tick, ahead of any dispatch.
A failed push never blocks the agent: commits accumulate locally and the next successful push carries them, subject to the origin rule in Scheduling (no *new* logical runs while origin is unreachable).
The durability boundary: locally accumulated commits survive process restarts but **not container replacement** -- which is exactly why a *blocked* sync (rewritten history, conflict) halts dispatch entirely rather than letting routines act on identities that exist only in the local container, waiting to be lost and re-run as duplicates.

**Why.** One invariant carries this whole section: **model-directed processes never write to a git worktree or git metadata -- they write to disposable staging that the supervisor validates and imports.**
That buys knowledge atomicity (the branch only ever contains validated output of completed runs, so a half-finished attempt can never corrupt the context future runs read), crash safety (recovery is always "discard staging, retry the pending run"), a collapsed attack surface (a routine cannot plant hooks, attributes, or symlinks in a tree where the deploy-key-holding supervisor runs git), and safe curation (the model can never clobber a human's uncommitted knowledge edits, because it never sees the real worktree).
Two commits per attempt is less tidy than one; correct crash recovery beats an aesthetically pure log.
Checking staging and copying it are one operation for the same reason the reservation is durable before the run: anything the supervisor learns about a directory the model process could still be writing is a fact about the past.
A validating walk followed by a trusting copy is two answers to the same question, and only the copy's answer decides what enters the branch -- so the copy asks.

## Shutdown: terminate fast, lean on at-least-once

**Decision.** On SIGTERM the supervisor stops launching routines, signals in-flight routine process groups (the same SIGTERM → grace → SIGKILL as a timeout), discards their staged knowledge copies, records the interrupted attempts and gives back their reservations so a redeploy costs no retries, makes a final knowledge commit-and-push, and exits.
There is no drain mode.
Recommended `docker stop` timeout: 30 seconds.

**Why.** Draining would hold every deploy hostage to the longest in-flight routine for a benefit the design already provides two other ways: an interrupted attempt leaves its pending run intact, so the same logical run retries on the next boot under the same run ID, and staged knowledge means it left nothing half-written behind.
Fast shutdown keeps redeploys boring -- seconds, not routine-timeout minutes -- at the cost of occasionally redoing work, which at-least-once semantics already told routine authors to tolerate.

## Runs are sandboxed -- and local runs use the production container

**Decision.** In production, every model process is filesystem-sandboxed with Landlock (Linux kernel 5.13+, V2 preferred with V1 fallback): the supervisor spawns it through a re-exec shim (`openroutines sandbox-exec`) that applies the rules to itself and then execs opencode -- rules bind to a process and its descendants, which is exactly what exec preserves.
The ruleset grants read on the run workspace, the OS paths needed by the runtime (including `/proc`), and the opencode installation; write on staged knowledge, the run tmp dir, a **disposable per-attempt HOME** inside the workspace, and `/dev`.
Landlock rules are additive -- a grant on a parent subsumes everything beneath it -- so the rule set must never contain an ancestor of the workspace: an early version granted `/tmp` wholesale, which silently made the entire workspace (the run's temp directory lives there) writable.
The per-attempt HOME exists because a *shared* writable opencode home is a cross-routine channel: opencode persists session state and auto-loads global plugins from its config dir (measured, not assumed), so one prompt-injected routine could plant state a later, more privileged routine would execute.
Each attempt now starts with a fresh HOME that dies with its workspace.

**The `/dev` grant is the same shape, still open**: `/dev/shm` is a tmpfs mounted inside a directory granted read-write, and Landlock resolves a path up through mount points, so the grant reaches across it and one attempt reads and rewrites what an earlier one left there.
The integration test below probes it and asserts `allowed` -- the current behavior recorded as an executable fact, which flips to `denied` the day the channel closes.
It is a lesser channel than the shared HOME was (no auto-load path executes what it finds there), and closing it is an open question rather than a decision -- see "World-writable roots on LSM-less hosts" below, which holds the full argument and the candidate closures.

`/proc` stays readable -- `/dev/fd` resolves through it and node reads its own cgroup files -- while the supervisor marks itself **non-dumpable** at boot, so same-UID reads of its `/proc/<pid>/environ` and ptrace attach fail the kernel's access check.
Non-dumpable does not survive `execve`, so the flag alone protects only the supervisor's own pid: every child it spawns -- git above all, which runs on the lease heartbeat roughly once a minute -- is given a **constructed, secret-free environment** rather than an inherited one, or its `/proc/<pid>/environ` would republish whatever the supervisor's did.
The recommended file-based production key delivery (`OPENROUTINES_MASTER_KEY_FILE` / `OPENROUTINES_DEPLOY_KEY_FILE`) keeps key values out of the process environment and points to files outside the sandbox rule set.
**The sandbox fails closed at boot, not mid-run**: `supervise` probes the identity transition at startup (via `openroutines sandbox-probe`) and refuses to start when it fails, with no override; the deliberately ugly `OPENROUTINES_UNSAFE_NO_SANDBOX=1` disables only Landlock, in the shim.
The gating guarantee is the per-attempt UID drop, not Landlock, which is why a Landlock-less host (Fly, minimal kernels) boots where it once refused.
The rule set is verified by a Linux-gated integration test that applies the production rules on a real kernel and asserts each boundary from a confined child, workspace-not-writable and supervisor-environ-not-readable included.
Outside production the shim is unnecessary: local runs confine the model process in the per-run container, and `OPENROUTINES_NATIVE=1` dev mode is an explicit unconfined opt-in.
(A `check`-side pre-deploy probe of the image remains on the hardening backlog.)

Locally, the container boundary separates the model-directed process from the host: opencode and everything it spawns execute inside the image's *runtime stage* (debian + git + pinned opencode; no `openroutines` binary needed) with the git-free run workspace mounted, and the container is discarded after the run.
In production the topology is different: the model process is a *child of the supervisor in the same container*, and the barriers between them are the per-attempt UID, the non-dumpable supervisor, file-based key delivery, clean-env construction, and Landlock where present -- not a container boundary.
Each attempt running under its own UID is what makes the separation structural rather than layered, and what keeps two *concurrent* attempts from reading each other.
In both topologies, repository content outside the assembled workspace is absent or filesystem-denied, and undeclared skills are not merely permission-denied -- only declared skills are copied into the workspace, so they do not exist there.
These properties assume the documented deployment and trusted-repository boundary in SECURITY.md.
Docker is therefore a prerequisite for `routines run` -- but not for `scaffold`, `configure`, `check`, or `routines new`, which stay native and instant; contributors with opencode installed can bypass the container with `OPENROUTINES_NATIVE=1`.

**Why.** Skill-permission gating is context hygiene, not a wall -- a routine with bash can `cat` any file it can reach.
Landlock turns the declare-or-it-doesn't-exist principle into kernel enforcement, the same way clean-env construction does for credentials: a third enforcement point, built from scratch per run.
Local container execution supplies the same runtime image, opencode version, constructed run environment, and assembled workspace used in production, while the outer isolation mechanism differs: a disposable container locally, Landlock inside the deployed container.
The volume mount keeps the dev loop fast (no rebuild per routine edit) and writes knowledge into the local worktree, where it stays until pushed.
What sandboxing still doesn't cover is network egress: a prompt-injected routine can exfiltrate anything it *legitimately holds* -- its declared credentials, the knowledge it can read, the files in its sandbox -- to any host it can reach.
Scoping caps what a routine holds; nothing yet caps where it can send.
That's why grants are **authority, not configuration**: declaring a skill or credential is handing the routine power to act and to leak.
Per-routine egress control (Landlock's TCP scoping landed in kernel 6.7; a proxy-based allowlist is the fuller answer) is the highest-value future tier -- see open questions.

## The required boundary is a per-attempt UID; Landlock is defense-in-depth

**Decision.** The boundary that production *requires* -- between the supervisor and the model process, and between concurrent model processes -- is Unix UID separation, not Landlock.
Each run slot owns one identity from a reserved pool of UIDs 20000--20031, matching the configurable concurrency ceiling of 32.
Manual `routines run` inside the container reserves one more identity, uid one past the concurrency ceiling and pre-created by the image, under a kernel lock: proven empty at acquisition (a dead predecessor drops its lock but can leave an escaped descendant) and reaped again at release.
Coordination with the supervisor is the knowledge worktree's own flock on `.openroutines-tmp/locks/knowledge.lock`, which a manual run takes around its staging snapshot and its settlement, so it can never become a second uncoordinated writer.
A manual settlement rides the container supervisor's lease and push; divergence is caught by the sync blockers rather than silently adopted.

The identity remains reserved through execution and settlement; before returning it to the pool, the supervisor enumerates `/proc` for every live process carrying that UID, kills descendants that escaped the attempt's process group, and verifies the UID is empty.
Failure poisons the identity, stops dispatch, cancels the other attempts, and exits so the deployment platform replaces the container -- reuse without proof would let an orphan read the next attempt assigned its UID.
The supervisor stays the non-root `agent` UID; its binary alone carries `CAP_SETUID`/`CAP_SETGID` to launch identities and `CAP_KILL` to reap them, and is executable only by `agent`.
When it forks a run, the trusted parent applies the target UID/GID before exec, and the child execs a separate world-executable, capless copy of the sandbox shim, which verifies it is already running as the requested identity, applies Landlock when the host provides it, and execs the capless opencode binary.
A routine therefore cannot re-execute the capability-bearing supervisor binary, and re-executing the helper supplies no privilege with which to change identity.
This parent-side transition also works under `no_new_privileges`, where relying on a child exec to acquire file capabilities would fail.
Boot probes the same parent-to-capless-helper path and refuses to supervise if the UID transition fails; `OPENROUTINES_UNSAFE_NO_SANDBOX=1` may disable Landlock but does not disable UID isolation.
The base image pre-creates the UID pool and strips every setuid/setgid bit before adding the narrowly scoped file capabilities.

**The attempt's filesystem access is granted on the group axis, never by changing ownership.**
Each identity's primary GID equals its UID, and the `agent` user belongs to every attempt group, so the supervisor -- owner of everything it stages -- flips each staged path's group to the attempt's group, an unprivileged operation for a group the owner belongs to: read-only group bits on the workspace, group-write on the three trees the attempt may mutate (staged knowledge, the run tmp, the attempt HOME), zero world bits.
The sandbox shim sets umask 007 before exec, so files the model process creates arrive group-accessible too: the supervisor can import and remove them while other attempt identities still see nothing.
The memberships are asserted at boot with the binary's own `CAP_SETGID`, not assumed from `/etc/group`: whether the image's `useradd -G` grant reaches the process depends on the init that started the container (Docker calls initgroups; minimal platform inits set only uid and gid and clear the supplementary groups), so the supervisor and manual runs join any missing attempt groups themselves and refuse to run only when that fails.
umask only removes bits -- a model process can still chmod its own files closed -- so a staged path the supervisor cannot read fails the import loudly, and a workspace it cannot delete is reclaimed by spawning the capless helper as the attempt identity to reopen the tree it owns, so a locked leftover cannot outlive the run and be read by the identity's next assignee.
This implements issue #76 and extends its single `routine` UID into the concurrency-safe pool required by PR #110.

**Decision (secrets).** A file-delivered master key in production must be mounted for the `agent` identity without group or other read bits; key loading rejects a broader mode rather than relying on Landlock to hide a world-readable secret.
The key is re-read when credentials are derived, so the delivery path remains available but unreadable to every attempt UID.
The deploy key must stay a file because `GIT_SSH_COMMAND` points `ssh -i` at a path, but its `0600` copy lives in the supervisor's `0700` home, owned by `agent`, unreadable to any attempt UID.
This closes the inversion #76 found: file-based key delivery was documented as the stronger option only because Landlock hid the files, so with the sandbox off a broadly mounted file-delivered key was *worse* than an env-delivered one; UID ownership and a fail-closed mode check now make file delivery genuinely stronger, as SECURITY.md claims.

**Why.** Every property the sandbox enforces -- the repo outside the workspace unreachable, `~/.ssh` and `/run/secrets/*` unreachable, the workspace read-only mid-run, the per-attempt HOME unpollutable, OS paths read-only -- is a statement about which UID may read or write which path, which is what Unix ownership has expressed since v7.
Requiring Landlock to state them excluded hosts `operating.md` names (Fly refuses to boot, and we deployed it with the one flag SECURITY.md says never to set), which made the promise false; UID separation states them on every kernel and, unlike Landlock, extends *between* concurrent attempts rather than only between the model and the host.
Landlock stays because it still buys two things UID ownership cannot: it can deny a process paths it *owns*, and it excludes the world-writable roots (`/tmp`, `/var/tmp`, `/dev/shm`, all `1777`) from the writable set.
Those roots are the one property that degrades on an LSM-less host under concurrency: ownership cannot lock a `1777` directory, so two attempts can pass data through them.
We narrow, not close: each attempt's `TMPDIR` and HOME point inside its own per-attempt-owned workspace, so nothing the runtime does by default lands in a shared root.
The residual is a passive data-passing channel between attempts, not a code-planting one, bounded by unguessable per-attempt paths -- the honest cost of booting where Landlock cannot.
Keeping Landlock as defense-in-depth costs nothing on hosts that have it and loses nothing where they don't: two independent layers, and boot requires only the one every host can provide.
Group grants rather than chown, because the first shipped cut chowned the attempt's trees to the attempt UID -- which quietly required `CAP_CHOWN`, then `CAP_FOWNER` for the chmod that followed each chown, then `CAP_DAC_OVERRIDE` to walk back into the 0700 tree it had just given away -- three capabilities the image never granted, so every containerized attempt died at staging while the host test suite, which skips every `OPENROUTINES_IN_CONTAINER` gate, stayed green.
`CAP_DAC_OVERRIDE` on a binary any `agent`-group process can execute bypasses most of what separates the supervisor from root; group membership expresses the same access matrix with the capabilities the binary already carries, and the smoke suite now runs the real staging path inside a container holding exactly the template's capability grant so that gap cannot reopen silently.

## The name: knowledge, not memory

**Decision.** What the agent builds as it works is **knowledge**, not memory: the `knowledge` branch, the `knowledge/` worktree, `knowledge.retention`, `routines run --write-knowledge`, and the vocabulary throughout code, docs, and injected instructions.
The rename is total, with no compatibility layer: an agent whose origin still carries a `memory` branch is migrated by hand (push `memory` as `knowledge`, delete `memory`, re-materialize the worktree), and an `openroutines.yml` still declaring `memory:` is rejected with the mapping, never silently reinterpreted.

**Why.** "Memory" is the most overloaded word in the agent ecosystem -- every chat assistant "remembers your preferences", every framework has a memory story -- and it undersells what the branch holds: a curated, versioned work record, not recall.
"Knowledge" is close to ownable and names what the thing becomes once events, tasks, context, and ledgers are curated rather than accumulated.
There is no migration machinery because alpha has zero external users -- the known agents were migrated by hand, which costs less than automation exercised exactly once.

## The name: OpenRoutines in prose, `openroutines` in code

**Decision.** The project is **OpenRoutines** in prose and `openroutines` in code, commands, package names, URLs, and other machine identifiers.
Never "Open Routines".
The repo, the domain, the Homebrew formula, the image, the environment-variable prefix, and the binary all stay the lowercase token; ORA is unchanged.

**Why.** One token still means the thing you type is the thing it's called -- the brand-to-binary mapping is capitalization, not a different name.
The spaced form collides with the product's own domain language ("routines" are the files; "open routines" mid-sentence is ordinary English), which is exactly why the joined name stays unambiguous.
The prose casing follows the upstream precedent: OpenCode uses **OpenCode** for the product and `opencode` for the command, and the two names sit side by side throughout these docs.
OpenRouter shares a prefix and an audience; we're keeping ours anyway, since it is a different layer entirely (they route model requests; we run agents).
...
sequenceDiagram
    ...
    participant St as Staged knowledge (no git)
    participant R as Routine attempt (opencode)
    participant W as Knowledge worktree (supervisor-only)
    participant O as Origin

    loop every minute
        S->>S: re-read frontmatter, find cron firings<br/>since the watermark, poll due triggers,<br/>or find a pending run to retry
    end
    Note over S: dispatched in due order into the bounded run pool;<br/>every knowledge write below takes the one knowledge lock
    ...
    S->>St: snapshot knowledge files into staging
    ...
    S->>S: export the attempt's sessions into operator storage<br/>(OPENROUTINES_SESSION_DIR)
    ...
    S->>O: push knowledge branch (retry later if it fails)
    S->>S: log the outcome, naming the sessions it left
```
...
- **World-writable roots on LSM-less hosts.** The `1777` roots (`/tmp`, `/var/tmp`, `/dev/shm`) are the residual from "The required boundary is a per-attempt UID": pointing each attempt's `TMPDIR`/HOME inside its own workspace keeps the runtime out of them by default, and a Landlock host still excludes them from the writable set, but on a kernel without Landlock two overlapping attempts can still pass data through a root that ownership cannot lock. Candidate closures: a private per-attempt `/dev/shm` and tmp mount (needs mount privilege the supervisor does not currently hold), or narrowing what the runtime is allowed to open there. Bounded meanwhile by unguessable per-attempt paths and the passive-channel-not-code-planting distinction.

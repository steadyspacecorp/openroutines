...
**Decision.** In production, every model process runs under its own reserved Unix identity, and a re-exec shim (`openroutines sandbox-exec`) applies Landlock when the host supports it before execing opencode -- rules bind to a process and its descendants, which is exactly what exec preserves.
...
**The required identity boundary fails closed at boot, not mid-run**: `supervise` probes the identity transition at startup (via `openroutines sandbox-probe`) and refuses to start when it fails, with no override; the deliberately ugly `OPENROUTINES_UNSAFE_NO_SANDBOX=1` disables only Landlock, in the shim.
...
Local container execution supplies the same runtime image, opencode version, constructed run environment, and assembled workspace used in production, while the outer isolation mechanism differs: a disposable container locally, a per-attempt UID plus Landlock where available inside the deployed container.
The volume mount keeps the dev loop fast (no rebuild per routine edit), while staging keeps the model away from the local knowledge worktree; only a `--write-knowledge` settlement imports its writes.
...
Requiring Landlock to state them excluded hosts `operating.md` names (Fly refused to boot, and we deployed it with the one flag SECURITY.md then said never to set), which made the promise false; UID separation states them on every kernel and, unlike Landlock, extends *between* concurrent attempts rather than only between the model and the host.
Landlock stays because it still buys two things UID ownership cannot: it can deny a process paths it *owns*, and it excludes `/tmp` and `/var/tmp` from the writable set.
World-writable roots are the one property that degrades without that layer under concurrency: ownership cannot lock a `1777` directory, so two attempts can pass data through `/tmp` or `/var/tmp`; `/dev/shm` remains reachable even with Landlock because the rules grant `/dev`.
...
sequenceDiagram
    ...
    S->>R: spawn model process behind the deployment boundary --<br/>per-run container locally, attempt UID in production;<br/>generated agent definition (declared skills only)

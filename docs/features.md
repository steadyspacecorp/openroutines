...
| **Disposable-memory runs** -- `routines run --no-memory` performs a real credentialed run but discards staged memory writes and the run record | Acting paths can be exercised against a scratch target without teaching the agent from the exercise; the flag is narrow enough not to imply that external actions are safe or simulated. |

...
| **Disposable-knowledge runs** -- `routines run --no-knowledge` performs a real credentialed run but discards staged knowledge writes and the run record | Acting paths can be exercised against a scratch target without teaching the agent from the exercise; the flag is narrow enough not to imply that external actions are safe or simulated. |
